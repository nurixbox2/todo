...
const renderToDos = () => {
    ...
    tasks.forEach(element => {
        ...
        trash.addEventListener('click', () => {
            let con = confirm('Действительно ли вы хотите удалить задание?')
            if (con === true) {
        ...
        edit.addEventListener('click', () => {
            let editText = prompt('Введите новый текст!')
            tasks.forEach(item => {
                if (item.id == element.id) {
                    let con = confirm('Действительно ли вы хотите заменить запись?')
                    if (con == true) {
